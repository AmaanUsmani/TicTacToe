...
public class PolicyIterationAgent extends Agent {
	...
	public void initRandomPolicy()
	{
		...
		for(Game g: policyValues.keySet())	//loop for taking a game state from the set of game states
		{
			if(g.isTerminal())
			{
				this.policyValues.put(g, 0.0);		//skips g if terminal
				continue;
			}
			newMove = g.getPossibleMoves(no.nextInt());
			curPolicy.put(g, newMove);		//changing the moves in the curPolicy hashmap to random moves
		}
	}
